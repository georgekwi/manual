...
\item Sum up all the \ttt{emptySeats} across all flights
  ...
  \end{verbatim}
  \emph{
    We should be seeing errors now.  These errors are false-positives due to DataSense.  You could potentially eliminate them by adding \ttt{payload..*return default []} to the right of every single \ttt{Array} function.
  }
\end{enumerate}
